...
| [`2026-08-27-kay-robbins-revised-events-bundle.md`](2026-08-27-kay-robbins-revised-events-bundle.md) | Drafted. The revised `events.tsv` and `events.json` Kay Robbins asked for at the 2026-08-25 meeting, with her four rulings: a pause is `Offset` then `Onset` and never `HED_0012527`, drop the `event_type` and `HED` columns, do not build a HED schema for SSTIM, and look at STIM BIDS. The attachments are in the directory beside it and the crosswalk has not absorbed the rulings yet. |
