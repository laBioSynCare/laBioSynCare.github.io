...
| DBpedia Archivo | ⚠️ **listed, serving a development snapshot — re-measured 2026-09-20** — the download is repaired and the content is the problem. `/download?o=https%3A//w3id.org/sstim&f=owl` now answers **200**, redirecting to Archivo's Leipzig file store (`akswnc7.informatik.uni-leipzig.de/dstreitmatter/archivo/w3id.org/sstim/2026.08.17-195305/`), which serves 2043064 bytes of RDF/XML where 2026-09-02 measured a 500. Those bytes carry `owl:versionInfo "0.16.0-dev"`: Archivo holds exactly two versions, both crawled 2026-08-17, and `https://w3id.org/sstim` has since served `0.17.0` with its own `owl:versionIRI`, so the public record presents a development line as the ontology. The Databus artifact is still absent, `https://databus.dbpedia.org/ontologies/w3id.org/sstim` **404** against a `foaf` control at 200, so the deploy step that failed at submission has never succeeded. Upstream the updater is dead rather than slow: [dbpedia/archivo#58](https://github.com/dbpedia/archivo/issues/58) reports no crawl of any ontology since 2026-02-23, [#55](https://github.com/dbpedia/archivo/issues/55) reports the front end down, and the code was last pushed 2024-08-26. Our own 2026-08-17 versions came from a manual add, and both `archivo.dbpedia.org/add` and `archivo.tools.dbpedia.org/add` answer 200 today | no | re-add to force a crawl of the release, verify the new version is not a `-dev` line, then report the Databus 404 upstream with this measurement |
