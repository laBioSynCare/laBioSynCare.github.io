...
and engagement activity added after that submission. It also carries audited
exact routes for the stable BSC catalog identities and every subject in the
current mutable real-agent aggregate; those rules still need a registry update
after their targets are deployed and verified.
...
| Every reviewed real `/specialist`, `/organization`, `/ecosystem-record/relationship`, `/ecosystem-record/activity`, and `/ecosystem-record/role` IRI in the exact route manifest | `https://biosyncare-lab.web.app/current.ttl` |
...
- **Instance dereferencing.** Every synthetic ecosystem subject and every
  current reviewed real ecosystem subject has an exact audited route to its
  owning dump. Exact static routes are now staged for the BSC framework, BSC
  ...
  instance files and need exact routes when their public identities stabilize.
