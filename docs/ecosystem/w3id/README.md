...
audited exact routes for every synthetic ecosystem agent, qualified relationship,
and engagement activity added after that submission; those rules still need to
be included in a registry update after their Pages fixture target is deployed.
...
| `https://w3id.org/sstim/specialist/synthetic-alex-rivera` | `/ontology/instances/ecosystem/fixtures/synthetic-ecosystem.ttl` |
| `https://w3id.org/sstim/organization/synthetic-aurora-lab` | `/ontology/instances/ecosystem/fixtures/synthetic-ecosystem.ttl` |
| `https://w3id.org/sstim/organization/synthetic-resonance-coop` | `/ontology/instances/ecosystem/fixtures/synthetic-ecosystem.ttl` |
| Six exact synthetic `/ecosystem-record/relationship/{id}` IRIs | `/ontology/instances/ecosystem/fixtures/synthetic-ecosystem.ttl` |
| Fourteen exact synthetic `/ecosystem-record/activity/{id}` IRIs | `/ontology/instances/ecosystem/fixtures/synthetic-ecosystem.ttl` |
...
curl -sIL -H 'Accept: text/turtle' https://w3id.org/sstim/specialist/synthetic-alex-rivera | grep -i location
curl -sIL -H 'Accept: text/turtle' https://w3id.org/sstim/ecosystem-record/relationship/alex-membership-aurora | grep -i location
...
- **Instance dereferencing.** Every synthetic ecosystem agent, relationship,
  and activity IRI has an exact audited route to the fixture graph. Exact rules
  for real `/organization/{id}`, `/specialist/{id}`, and
  `/ecosystem-record/{kind}/{id}` records are staged and audited with the
  private/local F4 draft. Because the registry requires a live target, submit
  them immediately after the Pages data file deploys and verify them before the
  identifiers are announced or added to discovery UI.
  Implementation/framework/reference IRIs (`/implementation/bsclab/...`,
  `/framework/bsc`, `/ref/...`) are not yet individually dereferenceable — they
  live inside bundled instance files. Add per-resource rules (or a data-dump
  convention) when those instances stabilize.
