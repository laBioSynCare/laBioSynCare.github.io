...
| [0040](0040-patch-studio-native-session-and-track-classes.md) | A patch is a first-class SSTIM object: add `sstim:Patch` and `sstim:Track` with four disjoint subtypes, widen 23 parameter domains to unions, and shape patches as authoring objects rather than catalog entries (resolves session-package findings S1, S2, V1) | Superseded by 0041 |
| [0041](0041-stimulus-description-layers-and-the-canonical-schema-gap.md) | Four description layers (stimulation / stimulus description / engine configuration / execution); withdraw `sstim:Patch`, redefine `sstim:Preset` as engine-dependent with schema profiles; record gap A (engine-independent stimulus description — possible for determinate and stochastic stimuli, impossible in principle for adaptive ones) and gap B (canonical schema); admit non-human, object and empty-space targets; small core plus adjunctive modules | Accepted |
...
| [0043](0043-sstim-core-profile-and-module-boundaries.md) | Define a small SSTIM Core Profile, optional concern modules, a Full compatibility profile, and one versioned module manifest; preserve term IRIs and implement manifest/closure gates before redistributing terms | Proposed |
