...
| Patch Studio | A portable `patch-studio-model-2` object and checksummed session package, with explicit model-1 import | Serialises and reloads faithfully and includes a partial SSTIM `Preset` projection plus loss report; it still has no catalogue-preset JSON converter (ADR 0026) |
...
| Migration | **Two-origin migration test** (`make migrate-test`), 15 assertions across two separate origins, plus model-1 → model-2 patch migration fixtures | Future schema revisions | ✅ Two independently deployed instances pass a migration test |
