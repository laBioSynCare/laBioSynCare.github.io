...
| Patch Studio | A portable `patch-studio-model-1` object and checksummed session package | Serialises and reloads faithfully and includes a partial SSTIM `Preset` projection plus loss report; it still has no catalogue-preset JSON converter (ADR 0026) |
