...
| Patch Studio | A portable `patch-studio-model-3` object and checksummed session package, with explicit model-1 and model-2 import | Serialises and reloads faithfully and includes a partial SSTIM `Preset` projection plus loss report; it still has no catalogue-preset JSON converter (ADR 0026) |
...
| Storage | **Patches, annotations and profile are all local-first by default and Firestore when signed in, behind one shared conformance suite** (§1.6) | Private-sync implementation (see §3.2), gated on a second identity provider | ✅ The same conformance suite passes against every storage implementation |
| Patch data | Portable `patch-studio-model-3` (§1.4), explicit model-1/model-2 migration, file import/export, URL-fragment sharing, **and a checksummed session package with a deterministic SSTIM projection** ([SESSION_PACKAGE.md](SESSION_PACKAGE.md)) | Producer-adjacent SHACL validation and exhaustive nested/discrete mapping accounting; S1/S2/V1 are resolved by ADR 0041 | ✅ A patch exported from instance A imports identically into instance B; `make session-conformance` proves Level 1 + Level 2 equivalence across two origins |
...
| Migration | **Two-origin migration test** (`make migrate-test`), 15 assertions across two separate origins, plus model-1/model-2 → model-3 patch migration fixtures | Future schema revisions | ✅ Two independently deployed instances pass a migration test |
