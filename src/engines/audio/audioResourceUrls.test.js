...
const mounted = path => `${deploymentBase}${path}`
...
describe('base-aware audio resources', () => {
  it('resolves the JavaScript worklet at the configured application base', async () => {
    ...
    expect(addModule).toHaveBeenCalledWith(mounted('/worklets/bsc-voice.worklet.js'))
  })
  ...
  it('resolves the WASM worklet and binary at the configured application base', async () => {
    ...
    expect(addModule).toHaveBeenCalledWith(mounted('/worklets/bsc-voice-wasm.worklet.js'))
    expect(fetchMock).toHaveBeenCalledWith(mounted('/worklets/bsc-osc.wasm'))
  ...
  it('keeps sample identifiers logical and resolves only at fetch time', async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(mounted('/audio/rain%20%26%20wind.wav'))
